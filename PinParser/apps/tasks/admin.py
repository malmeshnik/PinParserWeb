...
class ParseTaskAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ...
        ("Опції", {"fields": ("use_uniqueness", "uniqueness_config")}),
        (
            "Статус",
            {
                "fields": (
        ...
        ("Дати", {"fields": ("started_at", "finished_at")}),
    )
    ...
    def download_excel(self, obj):
        ...
        return format_html(
            '<a class="button" href="{}" download>⬇️ Excel</a>', obj.export_file.url
        )
    ...
    def view_results_link(self, obj):
        url = (
            reverse("admin:results_pinresult_changelist") + f"?task__id__exact={obj.id}"
        )
    ...
    def download_files(self, request, queryset):
        ...
        if files.count() == 1:
            ...
            response["Content-Disposition"] = (
                f'attachment; filename="{os.path.basename(task.export_file.name)}"'
            )
            return response
